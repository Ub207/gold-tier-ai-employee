...
SUCCESS_CRITERIA: [one line]"""
        ...
        print(f"    [OLLAMA] Error: {e} — using static defaults")
        return {
            "analysis": "Ollama unavailable — manual review needed.",
...
---"""
        ...
        print(f"    [OLLAMA] Error: {e} -- using static templates")
...
def process_drop(filepath: Path, meta: dict, dry: bool) -> dict:
    status = meta.get("status", "pending")
    if status in ("done",):
        return {"file": filepath.name, "action": "skipped"}
