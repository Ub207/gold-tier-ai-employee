...
def execute_linkedin_post(filepath: Path, meta: dict, body: str, dry: bool) -> dict:
    """
    LinkedIn posting is currently disabled (browser session issues).
    Files are logged and archived without opening browser.
    """
    print("    [SKIP] LinkedIn posting disabled — file archived.")
    return {"action": "linkedin_post", "status": "archived", "reason": "LinkedIn disabled by user"}
...
def execute_twitter_post(filepath: Path, meta: dict, body: str, dry: bool) -> dict:
    try:
        import tweepy
    except ImportError:
        return {"action": "twitter_post", "status": "error", "reason": "tweepy not installed. Run: pip install tweepy"}

    env = load_env()
    api_key    = env.get("TWITTER_API_KEY")
    api_secret = env.get("TWITTER_API_SECRET")
    acc_token  = env.get("TWITTER_ACCESS_TOKEN")
    acc_secret = env.get("TWITTER_ACCESS_TOKEN_SECRET")

    if not all([api_key, api_secret, acc_token, acc_secret]):
        return {"action": "twitter_post", "status": "error", "reason": "Twitter credentials missing in .env"}

    # Extract tweet text — strip frontmatter and approval checklist
    tweet_text = body
    if tweet_text.startswith("---"):
        parts = tweet_text.split("---", 2)
        if len(parts) >= 3:
            tweet_text = parts[2].strip()
    content_match = re.split(r"\n##\s+Approval", tweet_text, maxsplit=1, flags=re.IGNORECASE)
    tweet_text = content_match[0].strip()

    if len(tweet_text) < 5:
        return {"action": "twitter_post", "status": "skipped", "reason": "Tweet content is empty."}
    if len(tweet_text) > 280:
        tweet_text = tweet_text[:277] + "..."

    if dry:
        print(f"    [DRY] Would tweet ({len(tweet_text)} chars):\n    {tweet_text[:100]}...")
        return {"action": "twitter_post", "status": "dry_run"}
    ...
    try:
        ...
        return {"action": "twitter_post", "status": "posted", "tweet_id": tweet_id}
    except Exception as e:
        return {"action": "twitter_post", "status": "error", "reason": str(e)}


# ── Facebook / Instagram executor ──────────────────────────────────────────────

def execute_social_post(filepath: Path, meta: dict, body: str, dry: bool) -> dict:
    import urllib.request, urllib.parse, json as _json

    env = load_env()
    fb_token   = env.get("FACEBOOK_ACCESS_TOKEN")
    ...
    content_match = re.split(r"\n##\s+Approval", post_text, maxsplit=1, flags=re.IGNORECASE)
    post_text = content_match[0].strip()

    if len(post_text) < 5:
        return {"action": "social_post", "status": "skipped", "reason": "Post content is empty."}

    file_type = meta.get("type", "").lower()
    name = filepath.name
    post_to_fb = file_type in ("facebook_post", "social_post") or "FB" in name or "SOCIAL" in name
    post_to_ig = file_type == "instagram_post" or "IG" in name

    if dry:
        targets = []
        if post_to_fb: targets.append("Facebook")
        if post_to_ig: targets.append("Instagram")
        print(f"    [DRY] Would post to {', '.join(targets) or 'social'}:\n    {post_text[:100]}...")
        return {"action": "social_post", "status": "dry_run"}

    results = {}

    # Facebook post
    if post_to_fb and fb_token and fb_page_id:
        try:
            url = f"https://graph.facebook.com/v18.0/{fb_page_id}/feed"
            data = urllib.parse.urlencode({"message": post_text, "access_token": fb_token}).encode()
            req = urllib.request.Request(url, data=data, method="POST")
            with urllib.request.urlopen(req, timeout=30) as resp:
                resp_data = _json.loads(resp.read())
            results["facebook"] = resp_data.get("id", "posted")
            print(f"    [OK] Facebook posted! ID: {results['facebook']}")
        except Exception as e:
            results["facebook_error"] = str(e)
            print(f"    [ERROR] Facebook: {e}")

    # Instagram post (requires image_url — text-only not supported by basic API)
    if post_to_ig and ig_user_id and fb_token:
        image_url = meta.get("image_url", "")
        if not image_url:
            results["instagram"] = "skipped — image_url required for Instagram posts"
            print("    [SKIP] Instagram requires image_url in frontmatter.")
        else:
            try:
                # Step 1: create media container
                url1 = f"https://graph.facebook.com/v18.0/{ig_user_id}/media"
                data1 = urllib.parse.urlencode({"image_url": image_url, "caption": post_text, "access_token": fb_token}).encode()
                req1 = urllib.request.Request(url1, data=data1, method="POST")
                with urllib.request.urlopen(req1, timeout=30) as r1:
                    container = _json.loads(r1.read())
                container_id = container.get("id")
                # Step 2: publish
                url2 = f"https://graph.facebook.com/v18.0/{ig_user_id}/media_publish"
                data2 = urllib.parse.urlencode({"creation_id": container_id, "access_token": fb_token}).encode()
                req2 = urllib.request.Request(url2, data=data2, method="POST")
                with urllib.request.urlopen(req2, timeout=30) as r2:
                    pub = _json.loads(r2.read())
                results["instagram"] = pub.get("id", "posted")
                print(f"    [OK] Instagram posted! ID: {results['instagram']}")
            except Exception as e:
                results["instagram_error"] = str(e)
                print(f"    [ERROR] Instagram: {e}")

    has_success = any(k in results for k in ["facebook", "instagram"])
    has_error   = any("error" in k for k in results)
    status = "posted" if has_success else ("error" if has_error else "skipped")
    return {"action": "social_post", "status": status, "results": results}
...
def process_approved_file(filepath: Path, dry: bool) -> dict:
    ...
    elif file_type in ("twitter_post", "tweet") or name.startswith("TW_"):
        result = execute_twitter_post(filepath, meta, raw, dry)

    elif file_type in ("facebook_post", "instagram_post", "social_post") or name.startswith("SOCIAL_") or name.startswith("FB_") or name.startswith("IG_"):
        result = execute_social_post(filepath, meta, raw, dry)
    ...
    if status == "sent":
        ...
        if result.get("action") == "twitter_post":
            print(f"    [OK] Tweet posted! ID: {result.get('tweet_id', '')}")
        elif result.get("action") == "social_post":
            print(f"    [OK] Social post done: {result.get('results', {})}")
        else:
